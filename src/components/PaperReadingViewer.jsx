...
const PaperReadingViewer = ({ title, content }) => {
    ...
    const components = {
        h1: ({ node, ...props }) => (
            <h1 className="text-3xl font-extrabold text-slate-900 mt-20 pt-8 border-t border-slate-200 flex items-center gap-3" {...props}>
                <span className="w-2 h-8 bg-blue-600 rounded-full inline-block"></span>
                {props.children}
            </h1>
        ),
        h2: ({ node, ...props }) => (
            <h2 className="text-2xl font-bold text-slate-800 mt-16 pt-6 border-t border-slate-100 flex items-center gap-2" {...props}>
                <span className="text-blue-500 opacity-50">#</span>
                {props.children}
            </h2>
        ),
        h3: ({ node, ...props }) => (
            <h3 className="text-xl font-bold text-slate-700 mt-10 mb-4" {...props} />
        ),
